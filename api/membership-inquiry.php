<?php
/**
 * /api/membership-inquiry.php
 * Contact form handler for /services/membership lead inquiry modal.
...
$current_member_count = trim($data['current_member_count'] ?? '');
$current_software     = trim($data['current_software'] ?? '');
...
$check_text    = strtolower($name . ' ' . $message . ' ' . $org_name);
...
// Build email body
$body  = "New membership portal inquiry\n\n";
$body .= "--- CONTACT ---\n";
$body .= "Name: {$name}\n";
$body .= "Email: {$email}\n";
if ($phone) $body .= "Phone: {$phone}\n";
$body .= "\n--- ORGANIZATION ---\n";
$body .= "Org Name: {$org_name}\n";
if ($org_type)             $body .= "Org Type: {$org_type}\n";
if ($current_member_count) $body .= "Member Count: {$current_member_count}\n";
if ($current_software)     $body .= "Current Software: {$current_software}\n";
if ($message)              $body .= "\nMessage:\n{$message}";
...
if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_PHONE_NUMBER && ADMIN_PHONE) {
    try {
        $smsBody  = "New membership inquiry:\n";
        $smsBody .= "Name: {$name}\n";
        $smsBody .= "Org: {$org_name}\n";
        $smsBody .= "Email: {$email}\n";
        if ($current_member_count) $smsBody .= "Members: {$current_member_count}\n";
        if ($current_software)     $smsBody .= "Current: {$current_software}\n";
