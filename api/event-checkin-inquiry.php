<?php
/**
 * /api/event-checkin-inquiry.php
 * Contact form handler for /services/event-checkin lead inquiry modal.
...
$expected_attendees = trim($data['expected_attendees'] ?? '');
...
if (empty($expected_attendees)) $errors[] = 'Expected attendee count is required';
...
$check_text    = strtolower($name . ' ' . $message . ' ' . $event_name);
...
// Build email body
$body  = "New event check-in inquiry\n\n";
$body .= "--- CONTACT ---\n";
$body .= "Name: {$name}\n";
$body .= "Email: {$email}\n";
...
$body .= "Expected Attendees: {$expected_attendees}\n";
...
if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_PHONE_NUMBER && ADMIN_PHONE) {
    try {
        $smsBody  = "New event check-in inquiry:\n";
        $smsBody .= "Name: {$name}\n";
        $smsBody .= "Org: {$organization}\n";
        $smsBody .= "Event: {$event_name}\n";
        $smsBody .= "Date: {$event_date}\n";
        $smsBody .= "Attendees: {$expected_attendees}\n";
        $smsBody .= "Email: {$email}";
        if ($phone) $smsBody .= "\nPhone: {$phone}";
